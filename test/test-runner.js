...
function testSize() {
    ...
    $(".test").each(function () {
        ...
        toogleHeight = Number.parseFloat(toggle.match(/(\d+.\d+)/) || toggle.match(/(\d+)/));
        $(this).find(".row:eq(0) .col").each(function (idx) {
            if (idx == 0) return;
            item = $(this).find('div.toggle, .btn, [class^="form-control"]').css("height");
            itemHeight = Number.parseFloat(item.match(/(\d+.\d+)/) || item.match(/(\d+)/));
...
function testCustomSize() {
    ...
    $(".test").each(function () {
        ...
        toggleSize = Number.parseFloat(toggle.match(/(\d+.\d+)/) || toggle.match(/(\d+)/));
